...
int getEndIndex(double t){
  ...
  bool isWithinRange(int k, double t){
        //assert( k >= T.size() );
        if( T[k] <= t && T[k+1] > t) return true;
        return false;
    }
